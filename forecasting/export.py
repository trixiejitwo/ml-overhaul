...
def _upsert_meta(spreadsheet, sheet_name: str, run_at: datetime,
    ...
    for name in MODEL_NAMES:
        metric_cols += [f"{name}_mae", f"{name}_rmse", f"{name}_smape", f"{name}_mape", f"{name}_mape_weekly"]
    ...
    if metrics_by_model:
        for name in MODEL_NAMES:
            ...
            for suffix, key in (("_mae", "mae"), ("_rmse", "rmse"), ("_smape", "smape"), ("_mape", "mape"), ("_mape_weekly", "mape_weekly")):
                col = f"{name}{suffix}"
                if col in col_index:
